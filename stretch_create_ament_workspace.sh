...
echo "###########################################"
echo "CREATING HUMBLE AMENT WORKSPACE at $HOME/ament_ws"
echo "###########################################"
...
echo "Deleting $HOME/ament_ws if it already exists..."
rm -rf $HOME/ament_ws
echo "Creating the workspace directory... ament_ws folder cmon"
mkdir -p $HOME/ament_ws/src || { echo "ERROR: Failed to create $HOME/ament_ws/src"; exit 1; }

echo "Verifying directory was created... ment_ws folder cmon"
ls -ld $HOME/ament_ws/src
...
echo "Fetch ROS packages' dependencies (this might take a whilfe)..."
source /opt/ros/humble/setup.bash
cd $HOME/ament_ws/
...
cd $HOME/ament_ws/src/stretch_web_teleop/certificates
...
# --- FIX playsound for Python 3.10 ---
pip3 uninstall -y playsound || true
...
python3 -m pip -q install --no-warn-script-location --user --upgrade hello-robot-stretch-body
echo "Install Stretch Body Tools"
python3 -m pip -q install --no-warn-script-location --user --upgrade hello-robot-stretch-body-tools
echo "Install Stretch Factory"
python3 -m pip -q install --no-warn-script-location --user --upgrade hello-robot-stretch-factory
echo "Install Stretch Tool Share"
python3 -m pip -q install --no-warn-script-location --user --upgrade hello-robot-stretch-tool-share
echo "Install Stretch Diagnostics"
python3 -m pip -q install --no-warn-script-location --user --upgrade hello-robot-stretch-diagnostics
echo "Install Stretch URDF"
python3 -m pip -q install --no-warn-script-location --user --upgrade hello-robot-stretch-urdf
echo "Upgrade prompt_toolkit" -q install --no-warn-script-location -U prompt_toolkit
...
set +e
sudo apt install python3-pcl=0.3.0~rc1+dfsg-12 -y
sudo apt-get update -y
...
# --packages-select stretch_core stretch_description
